...
proc import file="/data/titanic_train.csv" out=train dbms=csv 
		replace;
run;

proc import file="/data/titanic_test.csv" out=test dbms=csv 
		replace;
run;
...
%LET SCORE_FILE '/notebooks/notebooks/Files_With_Hash_In_Name/SAS Model Sample (For Scheduler Monitor Testing)/mosaic_score1.sas';
